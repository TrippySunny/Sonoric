...
public sealed class PlaybackService : IDisposable
{
    ...
    private static void EnsureNative()
    {
        lock (NativeGate)
        {
            ...
            {
                foreach (var pluginDir in new[] { "/usr/lib64/vlc/plugins", "/usr/lib/vlc/plugins" })
                {
                ...
                NativeLibrary.SetDllImportResolver(typeof(Core).Assembly, ResolveLinux);
            }
    ...
    private static IntPtr ResolveLinux(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
    {
        if (!string.Equals(libraryName, "libvlc", StringComparison.OrdinalIgnoreCase) &&
